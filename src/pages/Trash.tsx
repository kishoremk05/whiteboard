...
export function TrashPage() {
  ...
  let filteredDeletedBoards = deletedBoards;
  if (searchQuery.trim()) {
    const lowerQuery = searchQuery.toLowerCase();
    filteredDeletedBoards = deletedBoards.filter(
      (board) =>
    ...
    <DashboardLayout searchQuery={searchQuery} onSearchChange={setSearchQuery}>
      ...
      {filteredDeletedBoards.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredDeletedBoards.map((board) => (
            <div
              key={board.id}
