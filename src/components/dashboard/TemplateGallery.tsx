...
import { Sparkles, GraduationCap, Briefcase, Lightbulb, Palette, FileText, Presentation, LayoutGrid, Check, ArrowRight, Eye, Users, Star, Zap, X } from 'lucide-react';
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
const categoryColors: Record<string, { bg: string; text: string; gradient: string }> = {
    general: { bg: 'bg-slate-100', text: 'text-slate-600', gradient: 'from-slate-400 to-slate-600' },
    education: { bg: 'bg-blue-100', text: 'text-blue-600', gradient: 'from-blue-400 to-blue-600' },
    business: { bg: 'bg-emerald-100', text: 'text-emerald-600', gradient: 'from-emerald-400 to-emerald-600' },
    creative: { bg: 'bg-rose-100', text: 'text-rose-600', gradient: 'from-rose-400 to-rose-600' },
};
                ...
                <div className="flex-1 overflow-hidden flex">
                    ...
                    <div className={cn(
                        ...
                        <div className={cn(
                            ...
                            previewTemplate ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-2 md:grid-cols-3"
                        )}>
                            {filteredTemplates.map((template, index) => {
                                const colors = categoryColors[template.category];
                                const stats = templateStats[template.id] || { uses: '1K+', rating: 4.5 };
                            ...
                            <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                                ...
                                <div className="aspect-video bg-gradient-to-br from-slate-100 to-slate-50 flex items-center justify-center">
                                    <div className={cn(
                                        ...
                                        {templateIcons[previewedTemplate.id] || <Sparkles className="w-10 h-10" />}
                                    </div>
                                </div>
                                ...
                                <div className="p-4">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <div className="flex items-center gap-1 text-xs text-slate-400">
                                            <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                                            <span>{templateStats[previewedTemplate.id]?.rating || 4.5}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-2 mb-4">
                                        {['Pre-configured layout', 'Customizable elements', 'AI-ready components'].map((feature, i) => (
